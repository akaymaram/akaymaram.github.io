# Development Projects
<li>Real-time Trade Intelligence Dashboard (avg. 3M shares of stocks and ETFs and $200M in mutual fund orders per day)</li>
<li>Usury Dashboard: Legal Interest Rate Choropleth (<a href="https://github.com/akaymaram/usury_dashboard">GitHub</a>)(<a href="https://usuryrates.streamlit.app">streamlit web app</a>)</li>
<li>Survival analysis + dimensionality reduction using <a href="https://umap-learn.readthedocs.io/en/latest/">UMAP</a> on 18M case records across 3 decades</li>
<li>Stress-level predictor using HRV feaures (ensemble supervised learning pipline)(<a href="https://github.com/akaymaram/ecg_stress_prediction/tree/main">GitHub</a>)</li>
<li>ECG signal processing peak detector (<a href="https://github.com/akaymaram/ecg_stress_prediction/tree/main/ecg_peak_finder">GitHub</a>)</li>
<li>Mammoth Eats, a full-stack cross-platform reservation app for Amherst College during covid-19 (<a href="https://apptopia.com/ios/app/1551664431/about">iOS version history</a>)</li>
<li>Isnad network visualization (digital humanities/graph data science)<br>
(<a href="graph.html">interactive network visualization using pyvis</a>)</li>
<li>Extractive text summarizer utilizing an evolutionary algorithm optimizer with pdf support (<a href="https://extractivesummarizer.streamlit.app">web access</a>)</li>